...
# メモ
# base = code.scan(/\d+/).map(&:to_i) 与えられたデータから係数の数値のみを抜き出し、配列に格納
# 考え方として、1文字ずつ確認し、数字が出現すれば出てきた係数を順に掛けていきkeyにスタック、文字が出現すればその掛けた係数分だけ文字が出現する。
# ")"が出現したタイミングで最後に掛けた係数でkeyを割る。ただし数値の後に()が続かない、1文字だけに掛かってる係数に注意する。
# 下記のコードで正しい結果が得られたが、もう少し可読性の高いコードを検討したい。
